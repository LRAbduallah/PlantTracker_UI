...
import { deleteFile, uploadFileToCloudinary } from "@/lib/services/image.service";
...
export const MultiImageUpload: React.FC<MultiImageUploadProps> = ({
      ...
      newFiles.forEach((newFile, index) => {
        ...
        const upload = async () => {
          try {
            ...
            if (newFile.url.startsWith("blob:")) URL.revokeObjectURL(newFile.url);
          }
        };
        upload();
      });
  ...
  const handleDeleteImage = useCallback(
    (id: string) => {
      ...
      const publicId = `uploads/${fileToDelete.url.split('uploads/')[1].split('.')[0]}`
      deleteFile(fileToDelete.publicId || publicId)
